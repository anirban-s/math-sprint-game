...
function updateBestScore() {
  bestScoreArray.forEach((score, index) => {
    // Select correct Best Score to udpate
...
// Stop the timer, process result
function checkTime() {
  if (playerGuessArray.length === questionAmount) {
    clearInterval(timer);
    // check for qrong guesses and add penalty time
...
// Add a tenth of a second to tomePlayed
function addTime() {
  timePlayed += 0.1;
  checkTime();
}
...
// Scroll, strore user selection in playerGuessArray
...
// Get the value fromthe selected radio button
